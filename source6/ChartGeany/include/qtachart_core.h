...
static Q_DECL_CONSTEXPR qint32 Y_PAD = 0;
static Q_DECL_CONSTEXPR qint32 SHADOW_WIDTH = 1;

#if QT_VERSION >= QT_VERSION_CHECK(5, 10, 0)
static Q_DECL_CONSTEXPR qint32 STRMAXLEN    = 256;
static Q_DECL_CONSTEXPR qint32 MAXITEMS     = 8192;
static Q_DECL_CONSTEXPR qint32 MAXGRIDLINES = 512;
#else
...
public:
  ...
  inline void setDefaultBackgroundColor (const QColor & color) Q_DECL_NOEXCEPT
  {
    background = color;
  }
...
public:
  ...
  int barOnX (const int x) const; // return the bar number that corresponds to x or -1 if not applicable

  void changeForeColor (QColor color); // change the foreground color
  void chartBackward (const int nbars);   // move the chart backward
  void chartBegin (void);   // move to chart's begin
  void chartEnd (void);     // move to chart's end
  void chartForward (const int nbars);    // move the chart forward by nbars
  void chartPageNext (void);    // move to previous page
  void chartPagePrevious (void);    // move to previous page
  void clearGRID (void) Q_DECL_NOEXCEPT; // clear all the grid lines on the chart
  void clearITEMS (void) Q_DECL_NOEXCEPT;  // clear all the rect objects on the chart
  ...
  void drawCandle (const QTAChartBarData *bardata, qint32 &counter);    // draw a candle on the chart
  ...
  inline qint32  getCurrentBar (void) const Q_DECL_NOEXCEPT
  {
    return currentbar;  // get the bar under mouse pointer
  }
  QString getBottomText (const int x); // form the bottom text
  QString getBottomTextHA (const int x); // form the bottom text for HA
  ...
  qreal MSVCFASTCALL priceOnY (const int y) const GNUHOT; // returns the price that corresponds to the y argument
  ...
  inline void setSizeChanged () Q_DECL_NOEXCEPT
  {
    size_changed = true;
  }; // set size_changed
  ...
  inline bool visibleLastBar () const Q_DECL_NOEXCEPT // report true if last bar is visible
  {
    return (bool) ((int) *excess_drag_width);
  }

  qreal MSVCFASTCALL yOnPrice (const qreal price) const GNUHOT; // returns the y that corresponds to the price argument
...
extern qreal maxfractionals (const FrameVector *HLOC) Q_DECL_NOEXCEPT GNUHOT;

// returns of core data of QTAChart
extern void *getData (QTAChart *) Q_DECL_NOEXCEPT;
