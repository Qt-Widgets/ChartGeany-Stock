...
{
public:
  LineEdge (void) Q_DECL_NOEXCEPT;             // constructor
...
class SubChartButton : public QToolButton
{
  Q_OBJECT
public:
  explicit SubChartButton (QTACObject *) Q_DECL_NOEXCEPT; // constructor
  ~SubChartButton (void); // destructor

  QTACObject *getOwner (void) const Q_DECL_NOEXCEPT; // get owner object
...
// functions
  ...
  void clearITEMS (void) Q_DECL_NOEXCEPT;  // clear all the objects on the chart
  ...
  QTACObject *getParentObject (void) Q_DECL_NOEXCEPT;  // return parent object
  QString getTitle (void) Q_DECL_NOEXCEPT;         // get object's title
  QString getTrailerCandleText (void) const; // get text's trailer candle
  QString getTrailerCandleText2 (void) const; // get text's trailer candle
  ...
  qreal   getPad (void) const; // get the edge's pad
  qreal   getPad2 (void) const; // get the edge's pad
  qint16  getTxtDirection (void) const; // get text's direction of label
  qint16  getTxtDirection2 (void) const; // get text's direction of label
  inline int getPeriod () const Q_DECL_NOEXCEPT
  {
    return period;  // get the period
  }

  inline DynParamsDialog *getParamDialog () const Q_DECL_NOEXCEPT
  {
  ...
  inline void setHAdjustment (int adj) Q_DECL_NOEXCEPT
  {
    hadjust = adj;
  }; // set text horizontal adjustment
  inline void setVAdjustment (int adj) Q_DECL_NOEXCEPT
  {
    vadjust = adj;
  ...
  inline void setThickness (int t) Q_DECL_NOEXCEPT
  {
  ...
  inline void setPrice (qreal p) Q_DECL_NOEXCEPT
  {
    price = p;  // set price
  }
  inline void setPeriod (int p) Q_DECL_NOEXCEPT
  {
    period = p;  // set period
  }
  void setSymbol (const char *symbol); // set symbol for text or label
  inline void setRange (qreal min, qreal max) Q_DECL_NOEXCEPT
  {
    rmin = min;
    rmax = max;
...
// functions
  ...
  inline qreal subYonPrice (const qreal price_arg) const Q_DECL_NOEXCEPT // returns the y of a subchart that corresponds to the price argument
  {
    return basey + ((height - 10) - ((price_arg + qAbs (rangemin)) / quantum));
  }
