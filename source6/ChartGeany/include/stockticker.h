...
class TickerWorker : public QObject
{
  Q_OBJECT
public:
  TickerWorker (void) Q_DECL_NOEXCEPT; // constructor
  ~TickerWorker (void);	// destructor
  inline bool isRunning () const Q_DECL_NOEXCEPT
  {
    return state;
  }; // returns running state
  
  void setParentObject (StockTicker *obj) Q_DECL_NOEXCEPT
  {
    parentObject = obj;
  }; // sets the parent
...
public:
  ...
  inline bool tickerRunning () const Q_DECL_NOEXCEPT
  {
    return ticker_running;
  }; // returns running state
  
  inline qint16 speed () const Q_DECL_NOEXCEPT
  {
    return tickerspeed;
  }; // returns ticker's speed
  
  inline void setSpeed (qint16 speed) Q_DECL_NOEXCEPT
  {
    tickerspeed = speed;
  }; // sets tickerspeed
