...
AddPriceDialog::ok_clicked ()
{
  ...
  const QString SQL =
    CGLiteral ("DELETE FROM prices WHERE symbol = '") %
    ui->symbolEdit->text ().trimmed () %
    CGLiteral ("' AND feed = 'NONE';") %
    CGLiteral ("INSERT INTO prices (symbol, price, change, volume, prcchange, date) VALUES ('") %
    ui->symbolEdit->text ().trimmed () % CGLiteral ("','") %
    ui->priceEdit->text ().trimmed () % CGLiteral ("','") %
    ui->changeEdit->text ().trimmed () % CGLiteral ("','") %
    ui->volumeEdit->text ().trimmed () % CGLiteral ("','") %
    QString::number ((qreal) cprc, 'f', 2) % CGLiteral ("%','") %
    ui->dateEdit->date ().toString ("yyyy-MM-dd") % CGLiteral ("');");
