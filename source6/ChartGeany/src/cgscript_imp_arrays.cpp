...
{
  QString id;              		// array id
  QVector <void *> element; 	// elements of the array
  unsigned int esize;          	// element size in bytes
  unsigned short maxdim;       	// maximum dimension
} Array;
...
ArrayReset_imp (void *arrptr)
{
  ...
  arr->element.push_back (nelem);

  if (CGSCRIPT_SANITIZER)
  {
    for (const auto r : *ArrayRegistry)
    {
      if (r->array == arr)
      {
...
ArrayCreate2_imp (const void *objptr, const char *id, int elemsize, int maxd)
{
  ...
  {
    for (const auto r : *ArrayRegistry)
      if (r->array->id == QString (id) && !r->array->id.isEmpty ())
        arr = r->array;
  }
...
ArrayDestroy_imp (void *arrptr)
{
  ...
  for (auto ptr : arr->element)
    ::operator delete (ptr);
  arr->element.clear ();

  delete arr;

  for (auto r : *ArrayRegistry)
  {
    if (r->array == arr)
    {
      if (CGSCRIPT_SANITIZER)
...
ArrayDestroyAll_imp (const void *objptr)
{
  ...
  int regsize = 0, counter = 0;
  do
  {
    regsize = ArrayRegistry->size ();
    if (regsize > 0)
    {
      counter = 0;

      while (counter < regsize && ArrayRegistry->at (counter)->objptr != objptr) 
        counter ++;
    
      if (counter < regsize)
        ArrayDestroy_imp (ArrayRegistry->at (counter)->array);  
    }    
