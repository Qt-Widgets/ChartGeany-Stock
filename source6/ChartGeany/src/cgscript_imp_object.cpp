...
ObjAddHorizonalLine_imp (void *ptr, ObjectHandler_t objptr)
{
  ...
  obj->setTitle (CGLiteral ("Horizontal Line"));
...
ObjAddVerticalLine_imp (void *ptr, ObjectHandler_t objptr, const char *id)
{
  ...
  obj->setTitle (CGLiteral ("Vertical Line"));
...
ObjAddCurve_imp (void *ptr, ObjectHandler_t objptr)
{
  ...
  obj->dynamic = true;
  obj->parentModule = pobj;
  obj->setTitle (CGLiteral ("Curve"));

  if (obj->paramDialog == nullptr) // this is a child
  {
    ParamVector pvector;
    obj->setParamDialog (pvector, "Curve");
    obj->paramDialog->addParam (CGLiteral ("Period"), CGLiteral ("Period"),
                                PARAM_TYPE::DPT_INT, obj->getPeriod ());
    obj->paramDialog->addParam (CGLiteral ("Color"), CGLiteral ("Color"),
  ...
  obj->setAttributes_common (QTACHART_CLOSE,
                             obj->period,
                             CGLiteral ("Period"), obj->rmin, obj->rmax,
                             obj->paramDialog->getParam (CGLiteral ("Color")),
                             CGLiteral ("Color"));
...
ObjAddVBars_imp (void *ptr, ObjectHandler_t objptr)
{
  ...
  obj->dynamic = true;
  obj->parentModule = pobj;
  obj->setTitle (CGLiteral ("Vertical Bars"));
  ...
  obj->setAttributes_common (QTACHART_CLOSE,
                             obj->period,
                             CGLiteral (""), pobj->rmin, pobj->rmax,
                             obj->paramDialog->getParam (CGLiteral ("")),
                             CGLiteral (""));
...
ObjSetText_imp (ObjectHandler_t objptr, String_t text)
{
  ...
  if (obj->type == QTACHART_OBJ_CURVE)
    // obj->title->setPlainText (obj->titlestr % CGLiteral ("(") % (obj->period > 0?QString::number (obj->period, 'f', 0):CGLiteral ("")) % CGLiteral (")") );
    obj->title->setHtml (obj->titlestr % CGLiteral ("(") % (obj->period > 0?QString::number (obj->period, 'f', 0):CGLiteral ("")) % CGLiteral (")") );
...
ObjSetColorRGB_imp (ObjectHandler_t objptr, int r, int g, int b)
{
  ...
  const QColor color = QColor (r, g, b, 255);
  ...
  else
  {
    ...
    if (pobj == nullptr) // not part of a subchart
    {
      const DynParamsDialog *dynparam = obj->getParamDialog ();
      ParamVector pvector = dynparam->getPVector ();
...
ObjSetInputVariables_imp (ObjectHandler_t objptr, Array_t InputVariableRegistry)
{
  ...
  const int dim = ArraySize_imp (InputVariableRegistry);

  for (int counter = 0; counter < dim; counter ++)
  {
    const _InputVariableRegistryRec *var = 
      (_InputVariableRegistryRec *) ArrayGet_imp (InputVariableRegistry, counter);
    
    const PARAM_TYPE type =
      (var->type == _VAR_INTEGER) ? PARAM_TYPE::DPT_INT :
      (var->type == _VAR_DOUBLE)  ? PARAM_TYPE::DPT_REAL :
      (var->type == _VAR_COLOR)   ? PARAM_TYPE::DPT_COLOR : 
                                    PARAM_TYPE::DPT_UNKNOWN;
    
    const QString name = QString (var->name);

    DynParam *param = new DynParam (name);
    param->type = type;
    param->show = var->show == 1 ? true : false;

    if (type == PARAM_TYPE::DPT_COLOR)
    {
      param->defvalue = QColor ((QRgb) ((Color_t) (var->defaultValue))).rgb ();
      param->callback_var = static_cast <void *> (var->colorVar);
    }
    else if (type == PARAM_TYPE::DPT_INT)
    {
      param->defvalue = var->defaultValue;
      param->callback_var = static_cast <void *> (var->intVar);
    }
    else if (type == PARAM_TYPE::DPT_REAL)
    {
      param->defvalue = var->defaultValue;
