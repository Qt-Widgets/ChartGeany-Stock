...
{
public:
  StringRegistryRecord (const QString qs = 0, const QString ids = 0)
  {
    if (ids == 0)
      id.truncate (0);
    else
  ...
  ~StringRegistryRecord ()
  {
    if (cstr != nullptr)
    {
      ...
      {
        const QTACObject *obj = QTACastFromConstVoid (objptr);
        if (obj != nullptr)
...
StrInit2_imp (const ObjectHandler_t objptr, const char *id, const char *initstr)
{
  ...
  if (StringRegistry != nullptr)
  {
    for (const auto r : *StringRegistry)
...
StringDestroy_imp (const ObjectHandler_t objptr, String_t str)
{
  ...
  const StringRegistryRecord *s = static_cast <StringRegistryRecord *> (str);
...
StrGetWithId_imp (const ObjectHandler_t objptr, const char *id)
{
  ...
  const StringRegistryType::size_type sz = StringRegistry->size ();
  StringRegistryType::size_type counter = 0;
  while (counter < sz)
  {
...
StrCpy_imp (String_t dst, String_t src)
{
  if (StringRegistry == nullptr)
  {
    ...
    if (CGSCRIPT_SANITIZER && StringRegistry)
    {
      const StringRegistryRecord *s = static_cast <StringRegistryRecord *> (src);
      const QTACObject *obj = QTACastFromConstVoid (s->objptr);
      if (obj != nullptr)
        obj->sanitizer->cgsInc ();
    }
  }
  ...
  StringRegistryRecord *d = static_cast <StringRegistryRecord *> (dst);
  StringRegistryRecord *s = static_cast <StringRegistryRecord *> (src);

  StringRegistryType::size_type idx = StringRegistry->indexOf (s);
  if (idx == -1)
...
StrCat_imp (String_t dst, String_t src)
{
  ...
  StringRegistryType::size_type idx2 = StringRegistry->indexOf (s);
  if (idx2 == -1)
    return static_cast <String_t> (d);

  StringRegistryType::size_type idx1 = StringRegistry->indexOf (d);
  if (idx1 == -1)
    d = StrRegister (s->objptr);
...
StrCmp_imp (String_t s1, String_t s2)
{
  ...
  StringRegistryRecord *sp1 = static_cast <StringRegistryRecord *> (s1);
  StringRegistryRecord *sp2 = static_cast <StringRegistryRecord *> (s2);

  const StringRegistryType::size_type idx1 = StringRegistry->indexOf (sp1);
  const StringRegistryType::size_type idx2 = StringRegistry->indexOf (sp2);
...
StrLen_imp (String_t str)
{
  ...
  StringRegistryRecord *strp1 = static_cast <StringRegistryRecord *> (str);
  const StringRegistryType::size_type idx = StringRegistry->indexOf (strp1);
...
Cstr2Str_imp (ObjectHandler_t objptr, String_t dst, const char *src)
{
  ...
  StringRegistryRecord *d = static_cast <StringRegistryRecord *> (dst);
  const StringRegistryType::size_type idx = StringRegistry->indexOf (d);
...
Str2NCstr_imp (char *cstr, String_t str, size_t n)
{
  ...
  StringRegistryRecord *s = static_cast <StringRegistryRecord *> (str);
  const StringRegistryType::size_type idx = StringRegistry->indexOf (s);
