                   ...
                   bool mode)                   // true for debug
{
  ...
  bool native =
    (Application_Settings->options.compiler.trimmed ().isEmpty ());
  ...
  {
    output +=
      CGLiteral ("compilation succeeded on: ") +
      QDateTime::currentDateTime().toString ("yyyy-MM-dd hh:mm:ss") +
      CGLiteral ("\n");

    result = createModule (program, programname, compileoutputfname);
    if (result == CG_ERR_OK)
      output += CGLiteral ("module created ");
    else
      output += CGLiteral ("module creation failed on: ") %
                QDateTime::currentDateTime().toString ("yyyy-MM-dd hh:mm:ss");
  }
  else if (native)
    output += CGLiteral ("compilation failed on: ") %
              QDateTime::currentDateTime().toString ("yyyy-MM-dd hh:mm:ss");
  ...
  dir.remove (compiletmpfname % CGLiteral (".o"));
...
Compile::onFinished(int code, QProcess::ExitStatus status)
{
  ...
  compilestatus = code;
  output.remove (compiletmpfname % CGLiteral (":"));
  output.replace ("`", "'");
  output.replace ("‘", "'");
  output.replace ("â", "");
  output.replace ("â", "");
