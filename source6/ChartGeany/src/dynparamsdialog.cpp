...
{
  const QString
  stylesheet = CGLiteral ("background-color: lightgray"),
  boxstylesheet = CGLiteral ("background-color: lightgray; color: black; border: 1px solid transparent;border-color: black;"),
  lblstylesheet = CGLiteral ("background-color: lightgray; color: black; border: 1px solid transparent;border-color: lightgray;"),
  chkbxstylesheet = CGLiteral ("border: 1px solid transparent; border-color: lightgray; color: black;");
...
DynParamsDialog::addParam (QString paramName, QString label,
                           PARAM_TYPE type, qreal defvalue)
{
  const QString
  stylesheet = CGLiteral ("background-color: lightgray"),
  lblstylesheet = CGLiteral ("background-color: lightgray; color: black; border: 1px solid transparent;border-color: lightgray;"),
  editstylesheet = CGLiteral ("background-color: white; color: black; border: 1px solid transparent; border-color: black;");
  QLabel *lbl;
  QLineEdit *edit;
  DynParam *param;
  QFont font;
...
void
DynParamsDialog::addParam (QString paramName, QString label,
                           PARAM_TYPE type, qreal defvalue, void *cbvar, bool show)
{
  ...
  if (cbvar != nullptr)
  {
    if (param->type == PARAM_TYPE::DPT_INT)
      param->value = param->defvalue = *(int *)param->callback_var;
    else if (param->type == PARAM_TYPE::DPT_REAL)
      param->value = param->defvalue = *(qreal *)param->callback_var;
    else if (param->type == PARAM_TYPE::DPT_COLOR)
...
DynParamsDialog::color_clicked (void)
{
  ...
  colorDialog->setGeometry(QStyle::alignedRect(Qt::LeftToRight, Qt::AlignCenter,
                           colorDialog->size(), availableGeometry (-1)));
  colorDialog->show ();
#ifdef Q_OS_MAC
  colorDialog->raise ();
#else
...
DynParamsDialog::function_accepted (void)
{
  ...
  foreach (const DynParam *param, Param)
  {
    if (param->callback_var != nullptr)
    {
      ...
      {
        if (param->type == PARAM_TYPE::DPT_COLOR)
...
DynParamsDialog::showEvent (QShowEvent * event)
{
  ...
  qint32 h = (Param.size () + 2) * param_height;
  ...
  foreach (DynParam *param, Param)
  {
    ...
    if (param->type == PARAM_TYPE::DPT_REAL)
    {
      if (param->callback_var != nullptr)
        param->value = param->defvalue = *(qreal *)param->callback_var;
