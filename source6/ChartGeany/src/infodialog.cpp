  ...
  QDialog (parent), ui (new Ui::InfoDialog)
{
  ...
  appname = APPNAME;
  version = CGLiteral ("Version ") %
             QString::number(VERSION_MAJOR,10) % CGLiteral (".") %
             QString::number(VERSION_MINOR,10) % CGLiteral (".") %
             QString::number(VERSION_PATCH,10);

  appurl = CGLiteral ("<html></style></head>") %
            CGLiteral ("<body style=\" font-family:'Sans Serif'; font-size:9pt;") %
            CGLiteral ("font-weight:600; font-style:normal;\">") %
            CGLiteral ("<p align=\"center\" style=\" margin-top:0px;") %
            CGLiteral ("margin-bottom:0px; margin-left:0px; margin-right:0px;") %
            CGLiteral ("-qt-block-indent:0; text-indent:0px;\">") %
            CGLiteral ("<a href=\"") % APPWEBPAGE %
            CGLiteral ("\">") %
            CGLiteral ("<span style=\" font-size:9pt; font-weight:bold;") %
            CGLiteral ("text-decoration: underline; color:#0000ff;\">") %
            APPWEBPAGE % CGLiteral ("</span></a></p></body></html>");
