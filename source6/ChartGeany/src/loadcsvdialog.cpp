  ...
  QDialog (parent), ui (new Ui::LoadCSVDialog)
{
  ...
  rc = selectfromdb (ComboItems->formats_query, sqlcb_formats, nullptr);
  ...
  rc = selectfromdb (ComboItems->timeframes_query, sqlcb_timeframes, nullptr);
  if (rc != SQLITE_OK)
  {
    setGlobalError(CG_ERR_DBACCESS, __FILE__, __LINE__);
    showMessage (errorMessage (CG_ERR_DBACCESS));
  ...
  rc = selectfromdb (ComboItems->markets_query, sqlcb_markets, nullptr);
...
{
  const QString title = CGLiteral ("Select CSV file");
  const QString wildcards = CGLiteral ("All files (*)");
  QString fileName = fileDialog->getOpenFileName(this, title, QDir::homePath (), wildcards);
  if (fileName != QLatin1String (""))
  {
    ui->filenameEdit->setText (fileName);
    if (fileName.endsWith (CGLiteral (".xls"), Qt::CaseInsensitive))
    {
      const int index =
        ui->formatComboBox->findText (CGLiteral ("EXCEL"), Qt::MatchContains);
      if (index != -1)
...
LoadCSVDialog::accept ()
{
  ...
  if (QLatin1String ("YAHOO CSV") == ui->formatComboBox->currentText () ||
      QLatin1String ("MICROSOFT EXCEL") == ui->formatComboBox->currentText ())
    symboldata.adjust = true;
  else
    symboldata.adjust = false;

  if (ui->marketEdit->text ().isEmpty ())
  ...
  */
  ...
  if (ui->fileposCheckBox->checkState () == Qt::Checked)
    ...
    symboldata.dnlstring.truncate (0);
