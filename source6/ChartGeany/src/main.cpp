...
void 
CGMessageHandler (QtMsgType type, const QMessageLogContext &context,
                  const QString &msg)
{
...
main (int argc, char *argv[])
{
  ...
  if (platformString ().contains (CGLiteral ("gcc")))
    path += QByteArray ("/gcc/bin;%path%");
  else if (platformString ().contains (CGLiteral ("pcc")))
    path +=  QByteArray ("/pcc/bin;/pcc/libexec;%path%");

  qputenv ("PATH", path);
  qputenv("QT_LOGGING_RULES","*.debug=false;qt.qpa.*=false"); // supress anoying messages

  if (installationPath.startsWith (CGLiteral ("C:/Program Files/WindowsApps/")))
    WinStore = true;
#endif
  ...
  {
    fprintf(stderr, "Cannot initialize TA-Lib (%d)!\n", retCode );
