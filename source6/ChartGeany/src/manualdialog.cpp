  ...
  QDialog (parent), ui (new Ui::ManualDialog)
{
  ...
  manualpath =
    QDir::homePath () % CGLiteral ("/") %
    CGLiteral (".config") % CGLiteral ("/") % APPDIR %
    CGLiteral ("/") % CGLiteral ("manual.html");
  ...
  while (!in.atEnd ())
  {
    html.replace (CGLiteral ("APPNAME"), APPNAME);
    html.replace (CGLiteral ("VERSION_MAJOR"), QString::number(VERSION_MAJOR));
    html.replace (CGLiteral ("VERSION_MINOR"), QString::number(VERSION_MINOR));
    html.replace (CGLiteral ("VERSION_PATCH"), QString::number(VERSION_PATCH));
    html.replace (CGLiteral ("APPWEBPAGE"), appwebpage);
    html.replace (CGLiteral ("DBNAME"), DBNAME);
    html.replace (CGLiteral ("APPDIR"), APPDIR);
    out << html << CGLiteral ("\n");
    html = in.readLine (0);
  }
  manual.close ();
