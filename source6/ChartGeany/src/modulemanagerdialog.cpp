  ...
  QDialog (parent), ui (new Ui::ModuleManagerDialog)
{
  ...
  stylesheet2 = CGLiteral ("background: transparent; background-color: white; color:black");

  cheadersList << CGLiteral ("  Name  ")
               << CGLiteral ("  Source ")
               << CGLiteral ("  Binary  ")
               << CGLiteral ("  Type  ")
               << CGLiteral ("  Author  ")
...
ModuleManagerDialog::reloadModules ()
{
  ...
  SQL = CGLiteral ("SELECT count (*) FROM modules");
  ...
  SQL = CGLiteral ("SELECT * FROM modules");
...
ModuleManagerDialog::attachModule (void *data, QString modid)
{
  ...
  if (status != "ENABLED" || platformString ().startsWith ("darwin") ||
      platformString ().startsWith ("sunos"))
  {
    Compile program;
    ...
    else
    {
      ...
      debugdialog->appendText (CGLiteral ("Compilation output for module ") % 
                               name % CGLiteral (":"));
  ...
  {
    debugdialog->appendText (CGLiteral ("Cannot open module: ") % id);
    return nullptr;
  }
...
ModuleManagerDialog::exportButton_clicked ()
{
  ...
  const QString SQL =
    CGLiteral ("DELETE FROM modules WHERE id = '") % id % CGLiteral ("';") %
    CGLiteral ("INSERT INTO modules (id, name, source, author, type, version, status, platform, binary) VALUES ('") %
    id % CGLiteral ("','") % name % CGLiteral ("','") %
    source % CGLiteral ("','") % author % CGLiteral ("','") %
    type % CGLiteral ("','") % version % CGLiteral ("','") %
    CGLiteral ("DISABLED', '', '');");
...
ModuleManagerDialog::deleteButton_clicked ()
{
  ...
  const QString SQL =
    CGLiteral ("DELETE FROM modules WHERE id = '") % id %
    CGLiteral ("';");
  rc = updatedb (SQL);
  if (rc != SQLITE_OK)
  {
