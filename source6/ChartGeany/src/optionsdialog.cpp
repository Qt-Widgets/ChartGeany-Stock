  ...
  QDialog (parent), ui (new Ui::OptionsDialog)
{
  ...
  ui->tabWidget->setStyleSheet(CGLiteral ("QTabBar::tab { height: 25px;}"));
  ...
  cheadersList << CGLiteral ("Symbol") << CGLiteral ("Feed");
  ...
  ui->tickerTableWidget->verticalScrollBar ()->setStyleSheet (CGLiteral ("background: transparent; background-color:lightgray;"));
  ui->tickerTableWidget->horizontalScrollBar ()->setStyleSheet (CGLiteral ("background: transparent; background-color:lightgray;"));
...
OptionsDialog::loadPlatforms ()
{
  ...
  SQL = CGLiteral ("SELECT * FROM toolchains WHERE platform LIKE '") %
        OS % BITS % CGLiteral ("%' ORDER BY platform;");
...
OptionsDialog::loadOptions (void)
{
  ...
  for (qint32 counter = 0; counter <ui->tabWidget->count (); counter ++)
  {
    ...
    foreach (const QObject *object, objectlist)
    {
      ...
      if (objName == QLatin1String ("tdapikeyEdit"))
        ((QLineEdit *) object)->setText (Application_Settings->options.tdapikey);
...
OptionsDialog::saveOptions ()
{
  ...
  for (qint32 counter = 0; counter <ui->tabWidget->count (); counter ++)
  {
    ...
    foreach (const QObject *object, objectlist)
    {
      ...
      if (object->objectName () == QLatin1String ("tdapikeyEdit"))
        Application_Settings->options.tdapikey = ((QLineEdit *) object)->text ().trimmed ();
...
OptionsDialog::symboladd_accepted ()
{
  ...
  if (ui->googleCheckBox->checkState () == Qt::Checked)
  {
    ...
    exists = IEX->symbolExistence (smbl, name, market);
    if (exists == false)
      add = showOkCancel ("Symbol does not exist. Add anyway?");
  }
  else
  {
    feed = CGLiteral ("YAHOO");
    valid = Yahoo->validSymbol (smbl);
    if (valid == false)
    {
      showMessage ("Invalid symbol.");
    ...
    exists = Yahoo->symbolExistence (smbl, name, market, currency);
    if (exists == false)
      add = showOkCancel ("Symbol does not exist. Add anyway?");
  }
