  ...
  QDialog (parent), ui (new Ui::PortfolioManagerDialog)
{
  ...
  cheadersList << CGLiteral ("  Title  ")
               << CGLiteral ("  Description  ")
               << CGLiteral ("  Currency  ")
               << CGLiteral ("  Feed  ")
               << CGLiteral ("  Data View  ")
               << CGLiteral ("  ID  ");
  ...
  rc = selectfromdb (ComboItems->transactiontypes_query, sqlcb_transactiontypes, nullptr);
  if (rc != SQLITE_OK)
  {
  ...
  rc = selectfromdb (ComboItems->commissiontypes_query, sqlcb_commissiontypes, nullptr);
  if (rc != SQLITE_OK)
  {
    setGlobalError(CG_ERR_DBACCESS, __FILE__, __LINE__);
    showMessage (errorMessage (CG_ERR_DBACCESS));
...
sqlcb_portfolios (void *classptr, int argc, char **argv, char **column)
{
  ...
  int rcount = 0;
   while (tw->item (rcount, 1)->text () != "Empty" && rcount < tw->rowCount ())
     rcount ++;
  ...
  for (qint32 counter = 0; counter < argc; counter ++)
  {
    ...
    if (colname == QLatin1String ("DATAVIEW"))
    {
...
PortfolioManagerDialog::reloadPortfolios ()
{
  ...
  SQL = CGLiteral ("SELECT count (*) FROM portfolios");
...
PortfolioManagerDialog::deleteButton_clicked ()
{
  ...
  QString SQL = CGLiteral (
    "DROP VIEW IF EXISTS V_NAME;\
     ...
     QLatin1String ("DELETE FROM portfolios WHERE pf_id = ") % QString::number (pf_id) % ";" %
     QLatin1String ("DELETE FROM transactions WHERE pf_id = ") %  QString::number (pf_id) % ";";
