...
QTAChartCore::QTAChartCore (QWidget * parent)
{
  ...
  typetitle = nullptr;
  bottom_text = nullptr;
  ruller_cursor = nullptr;
  expandBtn = nullptr;
  propertiesBtn = nullptr;
  functionBtn = nullptr;
  objectsBtn = nullptr;
  drawBtn = nullptr;
  helpBtn = nullptr;
  ...
  helpScr = nullptr;
  dataScr = nullptr;
  textobjectdialog = nullptr;
  lineobjectdialog = nullptr;
  chartEventFilter = nullptr;
  ...
  monthlit << CGLiteral ("JAN") << CGLiteral ("FEB") <<
           CGLiteral ("MAR") << CGLiteral ("APR") <<
           CGLiteral ("MAY") << CGLiteral ("JUN") <<
           CGLiteral ("JUL") << CGLiteral ("AUG") <<
           CGLiteral ("SEP") << CGLiteral ("OCT") <<
           CGLiteral ("NOV") << CGLiteral ("DEC");

  setSizeChanged ();

#ifdef DEBUG
  qDebug () << "core constructor: " << this;
#endif
...
QTAChartCore::~QTAChartCore (void)
{
  ...
  if (nbuttons == 4 && VOLUME.size () != 0 && gridstep > 1)
  {
...
{
  QTACObject *label, *found = nullptr;
...
{
  QTACObject *hline, *found = nullptr;
...
{
  QTACObject *vline, *found = nullptr;
...
{
  QTACObject *sline, *found = nullptr;
...
{
  QTACObject *text, *found = nullptr;

  // trailing text needs to be attached on a candle
  if (getBottomText (x).trimmed ().isEmpty ())
  {
    addLabel (item, x, y);
    return;
  }
...
int
QTAChartCore::barOnX (const int x) const
{
  if (nbars_on_chart == 0)
    return -1;
  ...
  const qreal b = framewidth * 1.5;
  const qreal a = x - chartleftmost;
  const qreal c = (chartwidth - a) / b;
  const int bar = qFloor (c);
...
QTAChartCore::changeForeColor (QColor color)
{
  ...
  if (tfBtn != nullptr && TIMEFRAME.size () == 4)
...
void
QTAChartCore::chartBackward (const int nbars)
{
  qreal k = *excess_drag_width;
  if (k > 0)
  {
  ...
  int maxstartbar = HLOC->size () - nbars_on_chart;
  if (maxstartbar < 0)
    maxstartbar = 0;
...
void
QTAChartCore::chartForward (const int nbars)
{
  if ((*startbar) >= nbars)
  {
    (*startbar) -= nbars;
...
// clear chart's GRID
void
QTAChartCore::clearGRID (void) Q_DECL_NOEXCEPT
{
...
// clear chart's ITEMS
void
QTAChartCore::clearITEMS (void) Q_DECL_NOEXCEPT
{
...
// vertical grid
       ...
       xcounter < max1 && xcounter < max2; xcounter ++, i++)
  {
    ...
    if (currenttf == QLatin1String ("DAY"))
    {
      ...
      {
        x = chartrightmost - ((qreal) (WIDTH * i));
        x -= (*excess_drag_width + 1);
        framestr = monthlit[pframe.month - 1] % CGLiteral (" ") % QString::number (pframe.year);
      }
    }
...
QString
QTAChartCore::getBottomText (const int x)
{
  QString btext = CGLiteral (" ");	
  static int last_bar = -1;
  const int bar = barOnX (x);
...
QString
QTAChartCore::getBottomTextHA (const int x)
{
  QString btext = CGLiteral (" ");
  static int last_bar = -1;
...
sqlcb_chart_setting (void *data, int argc, char **argv, char **column)
{
  ...
  else if (colname == QLatin1String ("edw"))
  {
    float *rsetting;
    rsetting = (float *) data;
    *rsetting = (float) strtod (argv[0], nullptr);
  }
  else
  {
    qint64 *isetting;
...
QTAChartCore::loadSettings ()
{
  ...
  // volume
  SQLCommand = CGLiteral ("SELECT VOLUME FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  ...
  // linear
  SQLCommand = CGLiteral ("SELECT LINEAR FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // online price
  SQLCommand = CGLiteral ("SELECT ONLINEPRICE FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // grid
  SQLCommand = CGLiteral ("SELECT GRID FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // chart style
  SQLCommand = CGLiteral ("SELECT CHARTSTYLE FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // frame width
  SQLCommand = CGLiteral ("SELECT FRAMEWIDTH FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // linecolor
  SQLCommand = CGLiteral ("SELECT LINECOLOR FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // barcolor
  SQLCommand = CGLiteral ("SELECT BARCOLOR FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // forecolor
  SQLCommand = CGLiteral ("SELECT FORECOLOR FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // backcolor
  SQLCommand = CGLiteral ("SELECT BACKCOLOR FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // startbar
  SQLCommand = CGLiteral ("SELECT STARTBAR FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
                     sqlcb_chart_setting, static_cast <void *> (&isetting));
  if (rc == SQLITE_OK)
  ...
  // excess drag width
  SQLCommand = CGLiteral ("SELECT EDW FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
  ...
  // time frame
  SQLCommand = CGLiteral ("SELECT TIMEFRAME FROM chart_settings WHERE KEY = '") %
               SymbolKey % CGLiteral ("';");
  rc = selectfromdb (SQLCommand.toUtf8(),
...
QTAChartCore::manageObjects (void)
{
  ...
  (CGLiteral ("background: transparent;color: white;font: 11px;"));
  expandBtn->setToolTip (TOOLTIP % CGLiteral ("Back (Alt+Z)</span>"));
...
qreal MSVCFASTCALL
QTAChartCore::priceOnY (const int y) const
{
  // linear chart
  if (linear)
  {
    const qint64 diff = (y - charttopmost);
    const qreal k = (qreal) diff / dots_per_point;
    const qreal price = points * ((qreal) charttop - k);
    return price;
  }

  // semilog chart
  const qreal bottom = qLn ((qreal) chartbottom * points);
  const qreal relpos = qAbs (((qreal) y - (qreal) chartbottomost) /
                             ((qreal) (chartbottomost - charttopmost)));
  const qreal k = (relpos *
        ((qLn (((qreal) charttop) * points) - bottom))) + bottom;
        
  const qreal price = qPow (2.718281,k);
...
QTAChartCore::saveSettings () const
{
  ...
  SQLCommand =
    CGLiteral ("DELETE FROM chart_settings WHERE KEY = '") %
    SymbolKey % CGLiteral ("';") %
    CGLiteral ("INSERT INTO chart_settings ") %
    CGLiteral ("(key, volume, linear, chartstyle, timeframe, onlineprice,") %
    CGLiteral (" linecolor, barcolor, framewidth, startbar, edw, ") %
    CGLiteral (" forecolor, backcolor, grid) ") %
    CGLiteral ("VALUES ('") % SymbolKey % CGLiteral ("',");

  // volume
  if (show_volumes)
    SQLCommand += CGLiteral ("1, ");
  else
  ...
  // chartstyle
  SQLCommand += QString::number (chart_style) % CGLiteral (", ") %

                // timeframe
                CGLiteral ("'") % currenttf % CGLiteral ("', ");
  ...
  // line color
  SQLCommand += QString::number ((qreal) linecolor.rgb (), 'f', 0) % CGLiteral (", ") %

                // bar color
                QString::number ((qreal) barcolor.rgb (), 'f', 0) % CGLiteral (", ") %

                // frame width
                QString::number (framewidth) % CGLiteral (", ") %

                // startbar
                QString::number (*startbar) % CGLiteral (", ") %

                // excess_drag_width
                QString::number (*excess_drag_width, 'f', 1) % CGLiteral (", ") %

                // foreground color
                QString::number ((qreal) forecolor.rgb (), 'f', 0) % CGLiteral (", ") %

                // background color
                QString::number ((qreal) backcolor.rgb (), 'f', 0) % CGLiteral (", ");
  ...
  SQLCommand +=
    CGLiteral ("DROP TABLE IF EXISTS template_") % SymbolKey % CGLiteral (";") %
    CGLiteral ("CREATE TABLE template_") % SymbolKey % CGLiteral (" AS ") %
    CGLiteral ("SELECT * FROM templatemodel;") % tmpldlg->qtachart2sql (SymbolKey);
...
QTAChartCore::setTitle (QString Title, QString Subtitle)
{
  ...
  if (Title.isEmpty ())
    Title = CGLiteral ("Unknown");
  if (Subtitle.isEmpty ())
    Subtitle = CGLiteral ("Unknown");
...
QTAChartCore::setChartProperties (void)
{
  events_enabled = false;
  hideAllItems ();
  expandicon = QIcon (CGLiteral (":/png/images/icons/PNG/Button_Back.png"));
  expandBtn->setIcon (expandicon);
  expandBtn->setStyleSheet
  (CGLiteral ("background: transparent;color: white;font: 11px;"));
  expandBtn->setToolTip (TOOLTIP % CGLiteral ("Back (Alt+Z)</span>"));
...
QTAChartCore::setRullerCursor (int y)
{
  ...
  qreal price = priceOnY (y);
  if (Q_UNLIKELY (price < 0))
    return;

  const QString pricestr = QString ("%1").arg (price, 10, 'f', fracdig + 1);
  ruller_cursor_y = y;
...
{
  if (Q_UNLIKELY (scene->items ().size () == 0))
    return;
...
QTAChartCore::tfBtn_clicked ()
{
       ...
       counter < maxcounter; counter ++)
  {
    if (TIMEFRAME.at(counter).TFSymbol == tfsymbol)
    {
      ...
      title = Symbol % CGLiteral (" - ") % TIMEFRAME.at (counter).TFName;
...
qreal MSVCFASTCALL
QTAChartCore::yOnPrice (const qreal price) const
{
  if (price == 0)
  ...
  if (linear)
  {
    const qreal k = price / points;
    const qreal xpoints = ((qreal) charttop - k) * dots_per_point;
    const qreal y = (qreal) charttopmost + xpoints;
    return y;
  }

  // semilog chart
  const qreal bottom = qLn (((qreal) chartbottom) * points);
  const qreal relpos = (qLn (price) - bottom) /
                       (qLn (((qreal) charttop) * points) - bottom);

  const qreal y = ((qreal) chartbottomost) -
