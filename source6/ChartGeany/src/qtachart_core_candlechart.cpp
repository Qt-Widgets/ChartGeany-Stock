...
static inline GNUPURE qreal BARWIDTH (const qreal width) 
{
  return width * 1.5;
}
...
{
  const FrameVector *hloc = (chart_style == QTACHART_CANDLE) ? HLOC : HEIKINASHI;
       ...
       k < maxk; k++, i++)
  {
    if (k < hlocsize)
    {
      const QTAChartFrame frame = hloc->at (k);
      const qreal yOpen  = yOnPrice (frame.Open),
                  yClose = yOnPrice (frame.Close),
                  yHigh  = yOnPrice (frame.High),
                  yLow   = yOnPrice (frame.Low);

      bardata.x = chartwidth - (BARWIDTH (bardata.width) * (i + 1)) + 1;
