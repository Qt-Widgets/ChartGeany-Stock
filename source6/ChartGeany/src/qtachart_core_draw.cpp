...
geom (QTAChartCore * core)
{
  ...
  */
  ...
  qreal gridlog = qLog10 (core->charttop - core->chartbottom);
  if (gridlog > 1) 
    gridlog --;

  core->gridstep = qPow (10, qCeil (gridlog));
  if (core->gridstep < 2)
    core->gridstep = 2;
...
QTAChartCore::draw (void)
{
  ...
  if (Q_UNLIKELY (!tfinit))
  {
    ...
    title = Symbol % CGLiteral (" - ") % TIMEFRAME[nbtn].TFName;
  ...
  if (reloaded)
  {
    ...
    OPEN.clear (); OPEN.reserve ((*HLOC).size ());
    CLOSE.clear (); CLOSE.reserve ((*HLOC).size ());
    HIGH.clear (); HIGH.reserve ((*HLOC).size ());
    LOW.clear (); LOW.reserve ((*HLOC).size ());
    VOLUME.clear (); VOLUME.reserve ((*HLOC).size ());
