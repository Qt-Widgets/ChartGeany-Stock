...
drawObject (QTACObject *object)
{
  ...
  {
    if (object->parentObject == nullptr) // object belongs to main chart
    {
      if (core->reloaded)
        object->valueSet ();
    ...
    if (object->valueset == nullptr)
      return;

    const qint32 vsetsize = object->valueset->size (),
  ...
  {
    QPen pen;
    quint32 chart_sequence = 0;

    if (core->reloaded)
      object->valueSet ();

    // no tooltip for title
    object->title->setToolTip (CGLiteral (""));
    ...
    const quint32 nobjects = core->Object.size ();
...
QTACObject::drawVBars ()
{
  ...
  const qreal y0 = subYonPrice (0);
  const qreal w = core->framewidth / ((3 * 0.85) / (thickness + 0.1));
  pen.setWidthF (w);

  const qint32 Ys = Y.size ();
  for (qint32 counter = 0; counter < Ys; counter ++)
  {
    const qreal x = X.at (counter);
    const qreal y = (y0 > Y.at (counter)) ? Y.at (counter) : y0;
    const qreal h = qAbs (Y.at (counter) - y0);
...
QTACObject::drawCurve ()
{
       ...
       counter < maxcounter; counter ++)
  {
    const QTAChartCore *core =
      static_cast <const QTAChartCore *> (chartdata);

    const qreal x2 = X.at (counter);
    const qreal y2 = Y.at (counter);
    ...
    if (Q_LIKELY(x1 > core->chartleftmost && x2 > core->chartleftmost))
      if (Q_LIKELY(y1 > basey && y1 < (basey + height)))
        if (Q_LIKELY(y2 > basey && y2 < (basey + height)))
        {
          ...
          {
            item = new QTCGraphicsItem (LineItemType);
            item->setLine (QLineF (x1, y1, x2, y2));
            ITEMS[icounter] = item;
            item->init (scene);
            item->setZValue (0.9);
...
QTACObject::drawDot ()
{
  ...
  for (qint32 counter = 0, maxcounter = Y.size ();
       counter < maxcounter; counter ++)
  {
    const qreal x1 = X.at (counter);
    const qreal y1 = Y.at (counter);
...
QTACObject::drawFibo ()
{
  ...
  {
    qreal x = 0, y = 0;
    const QRectF rectf = edge->pricetxt->boundingRect();
  ...
  for (qint32 counter = 0; counter < FiboLevelPrc.size (); counter ++)
  {
    ...
    const QRectF rectf = (const QRectF) FiboLevelPrcLbl[counter].boundingRect();
    ...
    if (counter > 0 && counter < FiboLevelPrc.size () - 1)
    {
      ...
      const QRectF rectf = (const QRectF) FiboLevelLbl[counter].boundingRect();
