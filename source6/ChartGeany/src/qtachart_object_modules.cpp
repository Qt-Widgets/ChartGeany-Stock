...
QTACObject::moduleInit (void *data, int *objtype)
{
  ...
  try
  {
    rslt = modinit (CGScriptFunctionRegistry_ptr (), CGScriptFunctionRegistrySize,
                    data, objtype, static_cast <void *> (this), nullptr);
    CHECK_SIGNAL
  }
  catch (std::exception& e)
  {
#ifdef DEBUG
    qDebug () << "Module '" % moduleName % "' reports " % QString (e.what ()) % " in Loop()" << "\n";
#endif

    debugdialog->appendText ("Module '" % moduleName % "' reports " % QString (e.what ()) % " in Init()");
    if (QString (e.what ()) == QLatin1String ("Floating-point exception"))
      rslt = CGSERR_FPE;
    else if (QString (e.what ()) == QLatin1String ("Segmentation fault"))
      rslt = CGSERR_SEGV;
    else if (QString (e.what ()) == QLatin1String ("Illegal instruction"))
      rslt = CGSERR_ILL;
    else
      rslt = CGSERR_SIGNAL;
  }
...
QTACObject::moduleEvent (int ev)
{
  ...
  {
#ifdef DEBUG
    qDebug () << "Module '" % moduleName % "' reports " % QString (e.what ()) % " in Event()" << "\n";
#endif
