...
{
  const QString
  stylesheet = CGLiteral ("color black; background-color: lightgray; border-color: black");
  cheadersList << CGLiteral ("Object") << CGLiteral ("Action");

  ui->setupUi (this);
  // ui->editButton->setStyleSheet ("background: transparent; border: 1px solid transparent;border-color: darkgray;color: white;");
  ui->editButton->setStyleSheet (CGLiteral ("background: transparent; color: white;"));
  ui->clearButton->setStyleSheet (CGLiteral ("background: transparent; color: white;"));
  ui->loadButton->setStyleSheet (CGLiteral ("background: transparent; color: white;"));
  ui->saveButton->setStyleSheet (CGLiteral ("background: transparent; color: white;"));
  ui->listWidget->setSelectionMode (QAbstractItemView::SingleSelection);
  ui->upToolButton->setToolTip (TOOLTIP % CGLiteral ("Scroll Up</span>"));
  ui->downToolButton->setToolTip (TOOLTIP % CGLiteral ("Scroll Down</span>"));
  ui->editButton->setToolTip (TOOLTIP % CGLiteral ("Edit Object</span>"));
  ui->clearButton->setToolTip (TOOLTIP % CGLiteral ("Clear All Objects</span>"));
  ui->loadButton->setToolTip (TOOLTIP % CGLiteral ("Attach Template</span>"));
  ui->saveButton->setToolTip (TOOLTIP % CGLiteral ("Save Template</span>"));
...
QTACObjects::loadObjectVector ()
{
  ...
  foreach (QTACObject *obj, core->Object)
  {
    if (!obj->deleteit)
    {
      ...
      title.remove (':');
      if (obj->moduleName != "")
        title = CGLiteral ("Module: ") %  title;
      if (!title.contains (CGLiteral ("Volume")))
      {
        if (obj->dynamic == false)
          ui->listWidget->addItem (title);
        else
          ui->listWidget->addItem (CGLiteral ("- ") % title);
        Object += obj;
      }
    }
  }
  ...
  if (ui->listWidget->count () == 0)
    ui->listWidget->addItem (CGLiteral ("No objects"));
}
...
QTACObjects::clearButton_clicked (void)
{
  ...
  if (showOkCancel (CGLiteral ("Clear all the objects?")) == true)
  {
