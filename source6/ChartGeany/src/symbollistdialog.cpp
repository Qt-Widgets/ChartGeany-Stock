  ...
  QDialog (parent), ui (new Ui::SymbolListDialog)
{
  const QString
  stylesheet =
    CGLiteral ("background: transparent; background-color: white;"),
    buttonstylesheet =
...
sqlcb_table_data (void *dummy, int argc, char **argv, char **column)
{
  ...
  for (qint32 counter = 0; counter < argc; counter ++)
  {
    ...
    else if (colname == QLatin1String ("CURRENCY"))
      s.currency = QString (argv[counter]);
    else if (colname == QLatin1String ("COUNTRY"))
      s.country = QString (argv[counter]);
    else if (colname == QLatin1String ("TYPE"))
      s.type = QString (argv[counter]);
  }
...
SymbolListDialog::reloadSymbols ()
{
  ...
  svector.clear ();
  query = CGLiteral ("SELECT * FROM ") % tablename %
          CGLiteral (" WHERE SYMBOL LIKE '")   % symFilter %
          CGLiteral ("%' ORDER BY SYMBOL;");

  rc = selectfromdb (query.toUtf8(), sqlcb_table_data, nullptr);
  if (rc != SQLITE_OK)
  {
  ...
  for (qint32 counter = 0; counter < svector.size (); counter++)
  {
    ...
    item = new QTableWidgetItem (QTableWidgetItem::Type);
    item->setText (svector[counter].currency);
    ui->tableWidget->setItem(counter,5,item);

    item = new QTableWidgetItem (QTableWidgetItem::Type);
    item->setText (svector[counter].type);
    ui->tableWidget->setItem(counter,6,item);

    item = new QTableWidgetItem (QTableWidgetItem::Type);
    item->setText (svector[counter].country);
    ui->tableWidget->setItem(counter,7,item);
  }
